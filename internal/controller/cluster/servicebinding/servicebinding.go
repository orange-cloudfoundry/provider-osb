...
import (
	...
	helpersv1alpha1 "github.com/orange-cloudfoundry/provider-osb/apis/cluster/helpers/v1alpha1"
...
func (c *external) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) { //nolint:gocyclo // See note below.
	...
	if err != nil {
		...
		return managed.ExternalObservation{}, fmt.Errorf("%w: %v", errOSBBindRequestFailed, err)
	}
	...
	if resp.Credentials == nil {
		return managed.ExternalObservation{}, fmt.Errorf("%w", errEmptyCredentials)
	}
...
func (c *external) Update(ctx context.Context, mg resource.Managed) (managed.ExternalUpdate, error) {
	...
	bindingData, err := helpersv1alpha1.GetDataFromServiceBinding(ctx, c.kube, binding)
	if err != nil {
		return managed.ExternalUpdate{}, fmt.Errorf("%s: %w", errCannotGetBindingData, err)
	}
	...
	if err != nil {
		return managed.ExternalUpdate{}, fmt.Errorf("%s: %w", errOSBRotatingRequestFailed, err)
	}
